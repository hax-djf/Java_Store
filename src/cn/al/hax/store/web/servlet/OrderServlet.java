...
import cn.al.hax.store.utils.PaymentUtil;
...
public class OrderServlet extends BaseServlet {
	...
	public String findMyOrdersWithPage(HttpServletRequest req, HttpServletResponse resp) throws Exception {
		/*
		 * 1这个时候需要实例化一个PageModel对象
		 * 2查询订单信息的时候是根据的一个用户对应着一个一个订单列表
		 * 3需要将里面的所有的订查询出来（在查询中需要传递一个num的参数表示当前页数）
		 */
		//获取用户信息
		User user=(User)req.getSession().getAttribute("loginUser");
		//获取当前页
	...
	public String payOrder(HttpServletRequest req, HttpServletResponse resp) throws Exception {
		...
		// 把付款所需要的参数准备好:
				...
				String p7_Pdesc = "";
				//接受响应参数的Servlet
				String p8_Url = "http://localhost:8080/Java_Store/OrderServlet?method=callBack";
				String p9_SAF = "";
				String pa_MP = "";
