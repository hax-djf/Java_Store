...
<!-- script脚本 -->
 ...
 function  _change() {
	/* 换图片操作 */
	var img=document.getElementById("srcMa");
	console.log(img);
	/* 给一个变量，点击改变路径操作，换一个参数，如果路径未变的话，这样的话，浏览器就默认认为是同一个提交操作 */
	img.src="/Java_Store/verifyStroke_Servlet?a="+new Date().getTime();
}
</script>
<body>
		...
		<form class="form-horizontal" style="margin-top:5px;" action="${pageContext.request.contextPath}/UserServlet?method=userRegist" method="post">
			...
			<label class="radio-inline">
			  ...
			  <div class="form-group">
			    ...
			    <div class="col-sm-2">
			    <img id="srcMa" src="${pageContext.request.contextPath}/verifyStroke_Servlet"/>
 	 			<a href="javascript:_change()">看不清</a>
    			<span>${requestScope.error.srcText}</span>
			    </div>
